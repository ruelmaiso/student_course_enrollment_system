...
if (isset($_GET['sid'], $_GET['cid'])) {
    $sid = (int)$_GET['sid'];
    $edit_student_sql = "SELECT student_id, name, allowance FROM students WHERE student_id = ?";
      ...
      <div class="card">
        ...
        <form method="post" action="enrollments_actions.php" id="enrollmentForm">
          ...
          <div class="grid">
            <div>
              <label>Student</label>
              <select class="select" name="student_id" <?php echo $edit ? 'readonly disabled' : 'required'; ?> onchange="updateStudentInfo()">
                <option value="">-- choose --</option>
                <?php if ($students) foreach($students as $s): ?>
                  <option value="<?php echo (int)$s['student_id']; ?>"
                    data-allowance="<?php echo $s['allowance']; ?>"
                    <?php echo $edit && (int)$edit['student_id']===(int)$s['student_id'] ? 'selected' : ''; ?>>
                    <?php echo htmlspecialchars($s['name']); ?> 
                    (Allowance: $<?php echo number_format($s['allowance'], 2); ?>)
                  </option>
                <?php endforeach; ?>
              </select>
              <?php if ($edit): ?>
            ...
            <div>
              <label>Course</label>
              <select class="select" name="course_id" <?php echo $edit ? 'readonly disabled' : 'required'; ?> onchange="updateCourseInfo()">
                <option value="">-- choose --</option>
                <?php if ($courses) while($c = $courses->fetch_assoc()): ?>
                  <option value="<?php echo (int)$c['course_id']; ?>"
                    ...
                    (Fee: $<?php echo number_format($c['course_fee'], 2); ?>)
                  </option>
                <?php endwhile; ?>
              </select>
              <?php if ($edit): ?>
          ...
          <div style="margin-top:12px; display:flex; gap:8px;">
            <button class="btn" type="submit"><?php echo $edit ? 'Update Date' : 'Enroll Student'; ?></button>
      ...
      <div class="card">
        ...
        <?php if ($enrollments && $enrollments->num_rows): ?>
          <table>
            <thead>
              <tr>
                <th>Student</th><th>Course</th><th>Fee</th><th>Allowance</th><th>Date</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
