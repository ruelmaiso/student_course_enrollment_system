...
$courses = $conn->query("SELECT course_id, course_name FROM courses ORDER BY course_id ASC");

$edit = null;
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $stmt = $conn->prepare("SELECT course_id, course_name FROM course WHERE course_id=?");
